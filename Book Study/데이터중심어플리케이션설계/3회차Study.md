...
* 선언형 질의언어가 많은사람들에게 선택되는 것이 설명됨
* 코드가 적으면서 최적화를 최적화기에 의해서 관리된다.
...
```
* 위의 예제에서 첫문장은 `바나나`와 `노란색`을 강조함. 
* 컴퓨터는 바나나와 노란색이 어떤 관계인지 인지 할 수 없음
* 두번째 문장은 트리플을 통해서 `바나나`는 `노랑`의 `색`을 가진다는 개념을 인지 할 수 있음
...
* 개인적으로는 그래프 모델의 쿼리는 SQL과 너무 동떨어져 있어서 대중성이 떨어짐!
